...
Dry reading: Insert the sensor in completely dry soil (or air) and record the raw ADC value → this represents 0% moisture.
Wet reading: Insert the sensor in fully water-saturated soil (or a cup of water) and record the raw ADC value → this represents 100% moisture.
...
Measure raw sensor output at reference point 1 (known low value, e.g. 0%).
Measure raw sensor output at reference point 2 (known high value, e.g. 100%).
Use linear interpolation (a straight line between these two points) to convert any raw reading in between into the corresponding real-world percentage.
